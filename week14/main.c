...
void checkDie(void)
{
    ...
    for (i=0;i<N_PLAYER;i++)
    {
        if (board_getBoardStatus(player_position[i]) == BOARDSTATUS_NOK && player_status[i] != PLAYERSTATUS_END)
        {
...
// ----- EX. 6 : game end ------------
	...
	if(live != 0)
	{
		printf("%i players are alive! winner is %s\n", getAlivePlayer(), player_name[getWinner(first)]);
	} else if (live == 0)
	{
		printf("There are no survivors, so there are no winners.");
	}
// ----- EX. 6 : game end ------------
